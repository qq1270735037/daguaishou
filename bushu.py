...
QYWX_AM = "wwf24ef724cd257c62,cnZV1yQbI-4n8bwBZ9ZMXkPdVbXIE53BzvZ1siM_wKE,@all,1000002,28vt3plK52yglzU5oXghvRcZMNd-x2R4Are0vUuRK3plHUOviV8UkEeyQwi8koKai"  # 企业微信
...
class WeCom:
    def __init__(self, corpid, corpsecret, agentid):
        self.CORPID = "xxxxxxxxxxxxxxx"
        self.CORPSECRET = "xxxxxxxxxxxxx"
        self.AGENTID = "xxxxxxxxxxxx"
...
def get_beijing_time():
    ...
    # 获取当前时间
    print(datetime.now().astimezone(target_timezone))
        ...
        if len(exec_results) >= PUSH_PLUS_MAX:
            ...
            for exec_result in exec_results:
                ...
                if success is not None and success is True:
                    html += f'账号：{exec_result["user"]}成功，{exec_result["msg"]}\n'
                else:
                    html += f'账号：{exec_result["user"]}失败，{exec_result["msg"]}\n'

        wecom_app(f"{format_now()} 刷步数通知", html)
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        try:
            config = dict(
                {
                    "USER": "xxxxxxxx#xxxxxxxxx#xxxxxxxxxxxxx",
                    "PWD": "xxxxxxxx#xxxxxxxxxxx#xxxxxxxxxxxxx",
                    "MIN_STEP": "19990",
                    "MAX_STEP": "23000",
                    "PUSH_PLUS_TOKEN": "",
                    "PUSH_PLUS_HOUR": "",
        ...
        min_step, max_step = int(config.get("MIN_STEP")), int(config.get("MAX_STEP"))
